...
class UserSerializer(serializers.ModelSerializer):
     ...
     def get_aprofilephoto_url(self, obj):
          request = self.context.get('request')
          if obj.aprofilephoto and hasattr(obj.aprofilephoto, 'url'):
               url = obj.aprofilephoto.url
               if request is not None:
                    return request.build_absolute_uri(url)
               return url
          return None

     def get_oprofilephoto_url(self, obj):
          request = self.context.get('request')
          if obj.oprofilephoto and hasattr(obj.oprofilephoto, 'url'):
               url = obj.oprofilephoto.url
               if request is not None:
                    return request.build_absolute_uri(url)
               return url
          return None
