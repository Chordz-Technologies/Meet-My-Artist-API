...
class UserAPI(ModelViewSet):
     ...
     def list(self, request, *args, **kwargs):
          try:
               ...
               serializer = self.get_serializer(user, many=True,
                                                context={'include_array_fields': True, 'request': request})
               api_response = {
                    'status': 'success',
     ...
     def retrieve(self, request, *args, **kwargs):
          try:
               ...
               serializer = self.get_serializer(instance, context={'include_array_fields': True, 'request': request})
               api_response = {
                    'status': 'success',
                    'code': status.HTTP_200_OK,
                    'message': 'User by ID',
...
class OrganizerByCategoryAPI(generics.ListAPIView):
     ...
     def list(self, request, *args, **kwargs):
          ...
          if category_obj:
               ...
               serializer = self.get_serializer(organisers, many=True, context={'request': request})
               data = serializer.data
               response_data = {
                    'status': 'success',
                    'code': status.HTTP_200_OK,
